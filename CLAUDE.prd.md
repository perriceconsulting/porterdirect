...
| **Retention** | **NOT BUILT — and worse than absent** | Deleting an order orphans the POD photo and signature in object storage **with no key left to find them**. That is PHI we can neither account for nor destroy on request. Under a BAA that is a reportable condition, not a backlog item. |
...
2. **The evidence spine**, in this order:
   ...
   2. **Retention, including an object-storage lifecycle rule.** Before access logging,
      because deleting an order currently orphans objects with no key left to find them,
      and *you cannot log access to objects you can no longer enumerate.*
